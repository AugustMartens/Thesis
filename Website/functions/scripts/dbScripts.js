...
async function db_insertion_google_calendar(email, accessToken, refreshToken, id) {
  const client = new MongoClient('mongodb+srv://Lennert:5YYRI2L2x3Gtlcu2@thesiscluster-zwbbo.gcp.mongodb.net/test?retryWrites=true&w=majority', { useNewUrlParser: true, useUnifiedTopology: true });
  await client.connect( err => {
    const collection = client.db("Thesis_db").collection("site_login");
    console.log("connected");
    collection.findOneAndUpdate(
...
exports.db_insertion_google_calendar = db_insertion_google_calendar;
